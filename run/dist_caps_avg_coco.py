...
'''
Compare the average caption feature distance for testing set MSCOCO
'''
...
annotations_path_ori = '../datasets/mscoco/coco_test1k.txt'
annotations_path_conf1 = '../datasets/mscoco/one_cap/coco_test_conf1.txt'
annotations_path_conf2 = '../datasets/mscoco/one_cap/coco_test_conf2.txt'
annotations_path_noth1 = '../datasets/mscoco/one_cap/coco_test_noth1.txt'
annotations_path_noth2 = '../datasets/mscoco/one_cap/coco_test_noth2.txt'
val_dataset = MSCOCODatasetRetFixConf(img_dir=MSCOCOCfgConf.img_dir, ann_file=annotations_path_ori, preprocess=preprocess, cap_num=5)
val_dataloader = DataLoader(dataset=val_dataset, batch_size=val_dataset.__len__())
# channel_snr=-18
random.seed(2)
# for channel_snr in [None, 18, 12, 6, 0, -6, -12, -18]:
for channel_snr in [24, -24]:
    ...
    for seed in range(4):
        ...
        for _, (images, captions) in enumerate(val_dataloader):
            ...
            with torch.no_grad():
                captions_1 = captions[0:5:]
                captions_tok = tokenizer(captions_1[0]).to(device)
